...
public class Minecraft implements IThreadListener, ISnooperInfo {
        ...
        if (!this.playerController.getIsHittingBlock()) {
            ...
            if (!this.thePlayer.isRowingBoat()) {
                for (EnumHand enumhand : EnumHand.values()) {
                    ...
                    if (itemstack1 == null && (this.objectMouseOver == null || this.objectMouseOver.typeOfHit == RayTraceResult.Type.MISS))
                        net.minecraftforge.common.ForgeHooks.onEmptyClick(this.thePlayer, enumhand);
    ...
    private void processKeyBinds() {
        for (; this.gameSettings.keyBindTogglePerspective.isPressed(); this.renderGlobal.setDisplayListEntitiesDirty()) {
            this.gameSettings.getCamera().onDeselected();
            ++this.gameSettings.selectedCameraIndex;

            if (this.gameSettings.selectedCameraIndex > this.gameSettings.cameras.size()) {
                this.gameSettings.selectedCameraIndex = 0;
            }
            this.gameSettings.getCamera().onSelected();

            if (this.gameSettings.getCamera().isFirstPerson()) {
