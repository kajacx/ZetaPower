    ...
    private void renderHand(float partialTicks, int pass) {
        if (!this.debugView) {
            ...
            if (!net.minecraftforge.client.ForgeHooksClient.renderFirstPersonHand(mc.renderGlobal, partialTicks, pass))
                if (this.mc.gameSettings.getCamera().isFirstPerson() && !flag && !this.mc.gameSettings.hideGUI && !this.mc.playerController.isSpectator()) {
                    this.enableLightmap();
            ...
            if (this.mc.gameSettings.getCamera().isFirstPerson() && !flag) {
